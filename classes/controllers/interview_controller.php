...
class interview_controller extends controller
{
    ...
    public function save_interview_action($params)
    {
        ...
        $data['meta'] = arr::extract($data['meta'], ['user_id', 'client', 'client_phone', 'calling_date', 'order_no', 'product', 'shop', 'answers_type']);
        $data['meta']['date'] = date('Y-m-d H:i:s');
        db::exec($this->db, "INSERT INTO i_interview_meta (date, user_id, client, client_phone, calling_date, order_no, product, shop, answers_type) "
