...
class users_controller extends controller
{
    ...
    {
        $params = arr::extract($_POST, array('name', 'password'));
        $params['msg'] = "please, sign up";

        if (arr::is_all_values_not_null($params)) {
            $count = db::exec_count($this->db, "SELECT COUNT(*) FROM i_users WHERE name = :name", array(':name' => $params['name']));
            if ($count > 0)
                $params['msg'] = "this name exists";
            else {
                db::exec($this->db, "INSERT INTO i_users (name, password) VALUES(:name, :password)", array(
                    ':name' => $params['name'],
    ...
    public function login_action($params)
    {
        ...
        if (arr::is_all_values_not_null($params)) {
            $count = db::exec_count($this->db, "SELECT COUNT(*) FROM i_users WHERE name = :name AND password = :password", array(
                ':name' => $params['name'],
                ':password' => md5($params['password'] . $this->salt)
            ));
    ...
    {
        $result = db::exec_row($this->db, "SELECT id, name FROM i_users WHERE name = :name", array(
